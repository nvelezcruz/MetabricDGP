...
OUTDIR = Path(os.environ.get("OUTDIR", str(BASE / f"output dir here")))
...
def run_single(seed=SEED):
    ...
    ICM_RANK = 6  # keep low (2–4). will be clamped to <= NUM_GROUPS below.

    NUM_INDUCING_SHARED = NUM_INDUCING
    NUM_INDUCING_OUT = NUM_INDUCING

    # Option 4: all hidden dims are shared representation
    ...
    class SharedLayer(gpytorch.models.ApproximateGP):
        """
        Shared GP over ALL patients: SHARED_DIM latent features.
        """
    ...
    class OutputLayerICM(gpytorch.models.ApproximateGP):
        """
        Multitask OUTPUT GP across NUM_GROUPS via LMC (ICM-style).
        Takes shared representation s (N, SHARED_DIM) and returns f for ALL tasks.

        UPDATED KERNEL:
        covar = Scale[ Matern(5/2, ARD) ]   (multitask via LMCVariationalStrategy)
        mean  = ZeroMean  (unchanged)
        """

        def __init__(self, in_dim, Xref, seed, num_groups, rank):
            num_groups = int(num_groups)
            rank = int(min(max(1, rank), num_groups))

            batch_shape = torch.Size([rank])  # latent funcs
            M = int(min(NUM_INDUCING_OUT, Xref.size(0)))

            inducing = (
                torch.randn(rank, M, in_dim, device=Xref.device, dtype=Xref.dtype)
                * 0.05
            )
            ...
            # >>> CHANGED HERE: Matern 5/2 ARD (private-layer style) instead of RBF+Linear
            self.covar_module = ScaleKernel(
                MaternKernel(nu=2.5, ard_num_dims=in_dim, batch_shape=batch_shape),
                batch_shape=batch_shape,
            )
            ...
            with torch.no_grad():
                try:
                    ...
                    # fallback if raw_lengthscale init differs
                    try:
    ...
    class DGP(gpytorch.models.deep_gps.DeepGP):
        ...
        def forward(self, x, g_idx):
            ...
            return self.output(s)  # multitask distribution over tasks
    ...
    model = DGP(
        ...
        seed=seed,
        num_groups=NUM_GROUPS,
        icm_rank=icm_rank_eff,  # <-- FIXED (was ICM_RANK)
        blocks=BLOCKS,
    ).to(device)
    ...
    def predict_risk_mc(X, g_idx, S=64, mode="linear"):
        ...
        with gpytorch.settings.cholesky_jitter(1e-3):
            for _ in range(S):
                f_all = model(X, g_idx).rsample()  # [N, G] (OPTION 4 multitask output)
                f = _select_task(f_all, g_idx)  # [N]
    ...
    # IMPORTANT: Keep FC MC separate from EVAL_MC_SAMPLES. EVAL_MC_SAMPLES is
    # intentionally small for routine metrics; FC needs more MC for stable rankings.
    FC_S = int(os.getenv("FC_SCORE_MC", "256"))
    ...
    # Optional runtime controls (leave unset to keep original behavior)
    #   FC_ONLY_SCOPES: comma-separated subset of {train,test,all} for FC runs
    ...
    #   FC_ONLY_SCOPES   = comma-separated subset of {train,test,all}
    #   FC_ONLY_HORIZONS = comma-separated months (e.g., '120,60')
    ...
    _fc_scopes = {"train", "test", "all"}
    if FC_ONLY_SCOPES:
        _fc_scopes = {s.strip().lower() for s in FC_ONLY_SCOPES.split(",") if s.strip()}
        _fc_scopes = _fc_scopes.intersection({"train", "test", "all"})
        if not _fc_scopes:
            _fc_scopes = {"train", "test", "all"}
