...
# Now adapted to multitask GROUP_LABELs (subtype×treatment groups)
# =============================================================================
...
# Fixed train/test split across all model seeds
SPLIT_SEED = 42
# ENDORSE-style split
TEST_FRAC = 0.50
...
OUTDIR = Path(os.environ.get("OUTDIR", str(BASE / "output_path_here")))
...
def run_single(seed=SEED):
    ...
    # --- Save split indices for reuse/verification (fixed across seeds) ---
    ...
    np.savez(
        ...
        tr_idx=tr_idx,
        va_idx=va_idx,
        patient_id=meta["PATIENT_ID"].astype(str).to_numpy(),
        y_strat=y_strat,
    )
    print(
    ...
    # Private representation dimensionality (per-group GP layer)
    PRIVATE_DIM = (
        3  # set small (e.g., 2–6); this layer is fit using each group's data only.
    )
    ...
    class PrivateLayerPerGroup(gpytorch.models.ApproximateGP):
        """
        Private GP layer for ONE group: PRIVATE_DIM latent features (no multitask kernel).

        This is evaluated on all X at forward-time, but gradients flow only through the
        samples whose g_idx selects this group's head downstream, so it effectively fits
        using that group's data.
        """
        ...
        def __init__(self, in_dim, private_dim, Xref, seed, blocks=None):
            ...
            # Robust fallback if Xref is empty for some reason
            if Xref is None or Xref.size(0) == 0:
                raise RuntimeError(
                    "PrivateLayerPerGroup requires non-empty Xref for inducing init."
                )
            ...
            if KMEANS_INIT and M >= 2:
                with torch.no_grad():
                    km = KMeans(n_clusters=M, random_state=int(seed), n_init=10)
                    centers = km.fit(Xref.detach().cpu().numpy()).cluster_centers_
                    inducing_base = torch.from_numpy(centers).to(
                        dtype=Xref.dtype, device=Xref.device
                    )
            ...
            # Kernel: same "type" as shared (Matern 5/2 ARD with optional additive blocks)
            if blocks and any(len(v) > 0 for v in blocks.values()):
                pieces = []
                for key in ("gene", "clin", "act", "cons", "ges"):
                    kblk = _mk_matern52_block(
            ...
            # Init lengthscale / outputscale
            with torch.no_grad():
                try:
                    base = getattr(self.covar_module, "base_kernel", None)
                    if base is not None and hasattr(base, "raw_lengthscale"):
                        base.raw_lengthscale.copy_(
                            torch.full_like(base.raw_lengthscale, inv_softplus(1.5))
                        )
                except Exception:
                    pass
                try:
                    self.covar_module.outputscale.fill_(1.5)
                except Exception:
                    pass
    ...
    class DGP(gpytorch.models.deep_gps.DeepGP):
        def __init__(
            self, in_dim, Xref, g_ref, seed, num_groups, icm_rank, blocks=None
        ):
            super().__init__()
            self.num_groups = int(num_groups)

            # Shared multitask layer (across groups)
            ...
            # Private layer per group (fit using that group's data via g_ref-driven inducing init)
            self.private = torch.nn.ModuleList([])
            g_ref_ok = (
                (g_ref is not None)
                and torch.is_tensor(g_ref)
                and (g_ref.numel() == Xref.size(0))
            )
            if g_ref_ok:
                g_ref_ = g_ref.to(device=Xref.device, dtype=torch.long).view(-1)
            else:
                g_ref_ = None

            for g in range(self.num_groups):
                if g_ref_ is not None:
                    idx = (g_ref_ == g).nonzero(as_tuple=False).squeeze(-1)
                    Xref_g = Xref.index_select(0, idx) if idx.numel() > 0 else Xref
                else:
                    Xref_g = Xref

                self.private.append(
                    PrivateLayerPerGroup(
                        in_dim=in_dim,
            ...
            # Private output heads (independent across groups) on concatenated [shared, private] representation
        ...
        def forward(self, x, g_idx):
            # Shared representation (task-selected from multitask shared layer)
            s = self.shared(x, g_idx)  # [N, SHARED_DIM]

            # Private representations for each group (evaluated at all x; only selected samples backprop)
            p_list = []
            for g in range(self.num_groups):
                p_g = (
                    self.private[g](x).rsample().transpose(-1, -2).contiguous()
                )  # [N, PRIVATE_DIM]
                p_list.append(p_g)
            p_all = torch.stack(p_list, dim=0)  # [G, N, PRIVATE_DIM]

            s_all = (
                s.unsqueeze(0).expand(self.num_groups, -1, -1).contiguous()
            )  # [G, N, SHARED_DIM]
            h = torch.cat([s_all, p_all], dim=-1)  # [G, N, SHARED_DIM + PRIVATE_DIM]

            # Batched private heads: batch_shape=[G], event_shape=[N]
            dist_b = self.output(h)
    ...
    def _rsample_f_all_NG(model, X, g_idx, num_groups, jitter=1e-3):
        """
        Sample multitask latent	register f_all for all N points and G tasks.
        Returns f_all shaped [N, G] (or [N,1] for scalar-output models).
        """
        with gpytorch.settings.cholesky_jitter(jitter):
            # Prefer full multitask output (all groups)
            try:
                dist = model(X)  # often returns multitask distribution [N,G]
            except TypeError:
                dist = model(X, g_idx)  # fallback if model requires g_idx
            f_all = dist.rsample()
        ...
        if f_all.dim() == 3:
            # If a singleton sample dim sneaks in, drop it
            if f_all.size(0) == 1:
                f_all = f_all.squeeze(0)
            else:
                raise RuntimeError(
                    f"_rsample_f_all_NG got unexpected 3D shape {tuple(f_all.shape)}"
                )
    ...
    def expected_cox_pll_mc(model, X, g_idx, time, event, S=MC_TRAIN):
        ...
        with gpytorch.settings.cholesky_jitter(1e-3):
            for _ in range(int(S)):
                f_all = model(X, g_idx).rsample()  # typically [N, G] (or [G, N])
    ...
    def total_kl_details(model):
        """
        KL decomposition for Workflow D (multitask shared + private per-group latent + private output heads):

            KL = sum_h KL_shared_dim[h]   (each is an LMC/ICM multitask GP)
               + sum_g KL_private[g]      (per-group private latent GP, batch_shape=[PRIVATE_DIM])
               + KL_output                (batched private heads, batch_shape=[G])
        """
        device0 = next(model.parameters()).device

        # Shared layer = SHARED_DIM separate multitask GPs
        kl_s = torch.tensor(0.0, device=device0)
        ...
        # Private latent layer = ModuleList of per-group ApproximateGPs
        kl_p = torch.tensor(0.0, device=device0)
        if hasattr(model, "private") and model.private is not None:
            for pgp in model.private:
                vs = getattr(pgp, "variational_strategy", None)
                if vs is None:
                    continue
                kl_p = kl_p + vs.kl_divergence().sum()
    ...
    for it in range(1, EPOCHS + 1):
        ...
        if it % PRINT_EVERY == 0 or it == 1:
            ...
            msg = (
                ...
                f"KLpr={(kl_p.item()/n_ev):.4f} | KLout={(kl_o.item()/n_ev):.4f} | beta={kl_beta:.3f}"
            )
            print(msg)
    ...
    try:
        ...
        df_tr_counts = _counts(g_tr_np, np.asarray(e_tr, int))
        df_tr_counts["split"] = "train"
        df_te_counts = _counts(g_te_np, np.asarray(e_te, int))
        df_te_counts["split"] = "test"
        df_counts = pd.concat([df_tr_counts, df_te_counts], ignore_index=True)
        df_counts.to_csv(
            archdir / f"archdiag_group_counts_E_seed{seed}.csv", index=False
        )
        ...
        rows_sim = []
        for gi, gname in enumerate(CANON_GROUPS):
            m = np.asarray(g_tr_np, int) == int(gi)
            n_g = int(m.sum())
        ...
        pd.DataFrame(rows_sim).to_csv(
            archdir / f"archdiag_train_cox_similarity_E_seed{seed}.csv", index=False
        )
    ...
    # ---------------- Posterior survival grouping at H ( existing block) ----------------------
    if USE_DISCRETE_TIME:
        print(
    ...
    def _run_fc_scope(scope_tag, X_s, g_np, t_np, e_np, nlS_vec_scope, Tsel_mat, H_INT):
        ...
        df.to_csv(
            outdir_fc
            / f"feature_collapsing_{_safe_fname(scope_tag)}_H{int(H_INT)}.csv",
            index=False,
        )
                ...
                if K_eff >= 1:
                    ...
                    try:
                        ...
                        for gi, gname in enumerate(CANON_GROUPS):
                            ...
                            # baseline / topK within-group C-index
                            c0_g, _ = harrell_c(t_arr[msk], risk0[msk], e_arr[msk])
                            ctop_g, _ = harrell_c(t_arr[msk], risk_top[msk], e_arr[msk])
                            dC_top_g = float(c0_g - ctop_g)

                            # random deltas within group
                            rand_dC_g = []
                            for rr, rvec in enumerate(risk_rand_list):
                                c_r_g, _ = harrell_c(
                                    t_arr[msk], np.asarray(rvec, float)[msk], e_arr[msk]
                                )
                                rand_dC_g.append(float(c0_g - c_r_g))
                                by_raw_rows.append(
                                    {
                                        "scope": str(scope_tag),
                                        "group_idx": int(gi),
                                        ...
                                        "K": int(K_eff),
                                        "rand_rep": int(rr),
                                        "deltaC_random": float(c0_g - c_r_g),
                                    }
                                )

                            rand_dC_g = np.asarray(rand_dC_g, float)
                            p_dc_g = (
                                float(np.mean(rand_dC_g >= dC_top_g))
                                if rand_dC_g.size
                                else np.nan
                            )
                            ...
                            row.update(
                                {
                                    "baseline_cindex": float(c0_g),
                                    "topK_cindex": float(ctop_g),
                                    "deltaC_topK": float(dC_top_g),
                        ...
                        if by_raw_rows:
                            pd.DataFrame(by_raw_rows).to_csv(
                                sens_dir
        ...
        _plot_topN_fc(
            ...
            outpath=outdir_fc
            / f"feature_collapsing_top10_{_safe_fname(scope_tag)}_H{int(H_INT)}.png",
            topN=10,
        ...
        for gi in range(min(K, len(GROUP_NAMES))):
            ...
            try:
                ...
                df_g.to_csv(
                    outdir_fc
                    / f"feature_collapsing_{_safe_fname(scope_g)}_H{int(H_INT)}.csv",
                    index=False,
                )
                ...
                _plot_topN_fc(
                    ...
                    outpath=outdir_fc
                    / f"feature_collapsing_top10_{_safe_fname(scope_g)}_H{int(H_INT)}.png",
                    topN=10,
                    xlabel="Feature Collapsing score (empirical Wasserstein-1; larger = more influence)",
                )
        ...
        pd.DataFrame(group_rows).to_csv(
            outdir_fc
            / f"feature_collapsing_{_safe_fname(scope_tag)}__per_group_summary_H{int(H_INT)}.csv",
            index=False,
        )
    ...
    try:
        ...
        if fc120 is None:
            fc120_path = outdir_fc / f"feature_collapsing_test_H{int(H_Q)}.csv"
...
if __name__ == "__main__":
    ...
    # Choose seeds once; do not hardcode; do not run seed 42 twice.
    ...
    rows = []
    for s in seeds:
        set_all_seeds(s)  # explicit (run_single also seeds internally)
        print(f"\n[Run] ===== Starting seed {s} =====")
    ...
    sd = df["c_test"].std()
    print(
        f"\nMulti-seed pooled test C-index (E[-log S(H_C)]): mean={mean:.3f} ± {sd:.3f} over {len(seeds)} seeds"
    )
    ...
    try:
        ...
        if sens_dir.exists():
            ...
            for base, paths in buckets.items():
                ...
                paths = [
                    p for p in paths if any(f"__seed{s}.csv" in p.name for s in seeds)
                ]
                if len(paths) == 0:
                    continue
    ...
    try:
        ...
        files = []
        for s in seeds:
            p = archdir / f"archdiag_train_cox_similarity_E_seed{s}.csv"
            if p.exists():
                files.append(pd.read_csv(p))
        ...
        if files:
            ...
            if "group" in all_df.columns and "cos_sim_beta_vs_pooled" in all_df.columns:
                ...
                agg.to_csv(
                    archdir / "archdiag_train_cox_similarity_E__mean_sd.csv",
                    index=False,
                )
